...
func (q *Queries) SearchItemsByName(ctx context.Context, query string) ([]SearchItemsByNameRow, error) {
	rows, err := q.db.QueryContext(ctx, searchItemsByName, query)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&i.Name, &i.StackSize, &i.FuelValue); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
